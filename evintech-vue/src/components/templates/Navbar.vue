...
import { isLoggedInUser, getLoggedInUserInfo, logOutUser } from "@/store/authentication";
import { onMounted, ref, toRefs } from "vue";
import { resetState, state } from "@/store";
import { getAuth, onAuthStateChanged, signOut } from "firebase/auth";
import router from "@/router";

let isUserLoggedIn = isLoggedInUser();
  ...
  <nav class="nav-container">
    ...
    <div class="nav-menu md:translate-x-0" :class="!isMenuOpen && 'translate-x-full'">
      <ul class="nav-menu-list">
        <MenuItem title="accueil" to="/" active />
        <MenuItem title="Messagerie" to="/messagerie"/>
        <MenuItem title="Favoris" to="/favoris"/>
        <MenuItem title="contactez-nous" to="/contact" />
        <MenuItem v-if="isUserLoggedIn && userInfo != null && userInfo.role.toLowerCase() === 'admin' || userInfo.role.toLowerCase() === 'organizer'" title="créer un évenement" to="/createevent" />
        <MenuItem v-if="isUserLoggedIn && userInfo != null && userInfo.role.toLowerCase() === 'admin'" title="Admin" to="/admin"/>
      </ul>
      <div class="nav-second-menu shadow-top md:shadow-none">
        ...
        >
          ...
          <router-link v-if="isLoggedIn && isUserLoggedIn" to="/profile" class="flex-centered signin-button">
            <User class="w-7 md:w-5 h-7 md:h-5" /> {{userInfo.userFirstName}} {{userInfo.userName}}
          </router-link>
          <router-link v-else to="/signin" class="flex-centered signin-button">
            <User class="w-7 md:w-5 h-7 md:h-5" />SE CONNECTER
          </router-link>
          <router-link v-if="isLoggedIn && isUserLoggedIn" @click="handleSignOut" to="/signin" class="flex-centered signin-button">
            SE DECONNECTER
          </router-link>
        </div>
