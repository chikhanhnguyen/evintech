...
const sendConfirmMail = () => {
  sendEmailVerification(auth.currentUser)
    ...
    window.alert("Mail de Vérification envoyé")
  });
}
...
<template>
  ...
  >
    ...
    <div v-if="!isVerified && isLoggedIn" class="mt-5" style="width:70%">
      <div class="col-sm-12 ">
        <div class="alert fade alert-simple alert-warning alert-dismissible text-left font__family-montserrat font__size-16 font__weight-light brk-library-rendered rendered show " role="alert" data-brk-library="component__alert">
          <button type="button" class="close font__size-18" data-dismiss="alert" aria-label="close">
              <span aria-hidden="true">
                <i class="fa fa-times warning"></i>
              </span>
          </button>
          <i class="start-icon fa fa-exclamation-triangle faa-flash animated"></i>
          <strong class="font__weight-semibold">Attention ! </strong> Veuillez vérifiez votre mail SVP<strong> (Vérifier les spams) |</strong>
          <button
            @click="sendConfirmMail"
            class=" hover:text-white"
            :class="!selected && 'text-white/40'"
          ><strong>&nbsp;&nbsp;Renvoyer un mail</strong></button>
        </div>
      </div>
    </div>
  </div>
